...
def create_config_file(config_name, config_content):
	...
	if os.path.isfile(config_file_path):
		...
		return {'status': 'error', 'message': '配置文件已存在' + config_name}
...
def get_configurations():
	...
	if api_channel not in api_channels:
		return jsonify({"error": "Invalid API channel"}), 400
...
def downloadConfig():
	...
	if api_channel == 'muhanfrp':
		...
		if response.status_code == 200:
			...
			return jsonify({'status': 'error', 'message': '请求错误'}), 400
		...
		return jsonify({'status': 'error', 'message': '未知的API渠道'}), 400
